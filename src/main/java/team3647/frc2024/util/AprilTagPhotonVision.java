...
public class AprilTagPhotonVision extends PhotonCamera implements AprilTagCamera {
    ...
    public Optional<Pose3d> camPose() {
        var update = photonPoseEstimator.update(this.getLatestResult(), this.getCameraMatrix(), this.getDistCoeffs());
        if (update.isEmpty()) {
    ...
    public Optional<VisionMeasurement> QueueToInputs() {
        ...
        var update = photonPoseEstimator.update(result, this.getCameraMatrix(), this.getDistCoeffs());
        if (update.isEmpty()) {
            return Optional.empty();
        }
